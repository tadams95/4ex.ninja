...
class MovingAverageCrossStrategy_EUR_USD_D:
    def __init__(
        self,
        slow_ma: int = 20,  # Changed from 60
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["EUR_USD_D_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_EUR_USD_D()
...
df_EURUSD_D["open"] = df_EURUSD_D["mid"].apply(lambda x: x["o"])
df_EURUSD_D["high"] = df_EURUSD_D["mid"].apply(lambda x: x["h"])
df_EURUSD_D["low"] = df_EURUSD_D["mid"].apply(lambda x: x["l"])
df_EURUSD_D["close"] = df_EURUSD_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_EURUSD_D = df_EURUSD_D[["open", "high", "low", "close"]]
...
# MA optimized to EUR_USD_D
ma_pair = [(20, 10)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_EURUSD_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_EUR_USD_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_EURUSD_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["EUR_USD_D_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            "time": index,
            "instrument": "EUR_USD",  # Changed from AUD_USD
            "timeframe": "D",
            "close": float(row["close"]),
            "SMA_20": float(row["slow_ma"]),  # Changed from SMA_60
