...
collection = db["GBP_JPY_D"]

collection_name = "GBP_JPY_D"
...
class MovingAverageCrossStrategy_GBP_JPY_D:
    def __init__(
        self,
        slow_ma: int = 160,  # Changed from 20
        fast_ma: int = 20,   # Changed from 10
        atr_period: int = 14,
        sl_atr_multiplier: float = 1.5,
        tp_atr_multiplier: float = 2.0,
        min_atr_value: float = 0.03,    # Adjusted for JPY pairs
    ...
    def generate_trade_dict(self, row: pd.Series) -> Optional[Dict]:
        ...
        try:
            ...
            # Calculate actual pip values for SL and TP (adjusted for JPY pairs)
            sl_pips = abs(close_price - stop_loss) * Decimal("100")  # JPY pairs move in 0.01 increments
            tp_pips = abs(take_profit - close_price) * Decimal("100")  # JPY pairs move in 0.01 increments
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["GBP_JPY_D_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_GBP_JPY_D()
    logging.info("Starting GBP/JPY Daily strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_GBPJPY_D = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_GBPJPY_D.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_GBPJPY_D["open"] = df_GBPJPY_D["mid"].apply(lambda x: x["o"])
df_GBPJPY_D["high"] = df_GBPJPY_D["mid"].apply(lambda x: x["h"])
df_GBPJPY_D["low"] = df_GBPJPY_D["mid"].apply(lambda x: x["l"])
df_GBPJPY_D["close"] = df_GBPJPY_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_GBPJPY_D = df_GBPJPY_D[["open", "high", "low", "close"]]
...
# MA optimized to GBP_JPY_D
ma_pair = [(160, 20)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_GBPJPY_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_GBP_JPY_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_GBPJPY_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["GBP_JPY_D_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            "time": index,
            "instrument": "GBP_JPY",  # Changed from AUD_USD
            "timeframe": "D",
            "close": float(row["close"]),
            "SMA_160": float(row["slow_ma"]),  # Changed from SMA_60
            "SMA_20": float(row["fast_ma"]),  # Changed from SMA_40
            "signal": int(row["signal"]),
