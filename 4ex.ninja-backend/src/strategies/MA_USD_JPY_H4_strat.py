...
collection = db["USD_JPY_H4"]

collection_name = "USD_JPY_H4"
...
class MovingAverageCrossStrategy_USD_JPY_H4:
    def __init__(
        self,
        slow_ma: int = 140,  # Changed from 70
        fast_ma: int = 40,   # Changed from 10
        atr_period: int = 14,
    ...
    def generate_trade_dict(self, row: pd.Series) -> Optional[Dict]:
        ...
        try:
            ...
            # Calculate pip values for JPY pairs (0.01 increments)
            sl_pips = abs(close_price - stop_loss) * Decimal("100")
            tp_pips = abs(take_profit - close_price) * Decimal("100")
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["USD_JPY_H4_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_USD_JPY_H4()
    logging.info("Starting USD/JPY H4 strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_USDJPY_H4 = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_USDJPY_H4.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_USDJPY_H4["open"] = df_USDJPY_H4["mid"].apply(lambda x: x["o"])
df_USDJPY_H4["high"] = df_USDJPY_H4["mid"].apply(lambda x: x["h"])
df_USDJPY_H4["low"] = df_USDJPY_H4["mid"].apply(lambda x: x["l"])
df_USDJPY_H4["close"] = df_USDJPY_H4["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_USDJPY_H4 = df_USDJPY_H4[["open", "high", "low", "close"]]
...
# MA optimized to USD_JPY_H4
ma_pair = [(40, 140)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_USDJPY_H4, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_USD_JPY_H4()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_USDJPY_H4)

# Create a new collection for signals if it doesn't exist
signals_collection = db["USD_JPY_H4_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            ...
            "SMA_140": float(row["slow_ma"]),  # Changed from SMA_70
            "SMA_40": float(row["fast_ma"]),   # Changed from SMA_10
            "signal": int(row["signal"]),
