...
class MovingAverageCrossStrategy_GBP_JPY_H4:
    def __init__(
        self,
        slow_ma: int = 70,    # Changed from 140
        fast_ma: int = 10,    # Changed from 40
        atr_period: int = 14,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["GBP_JPY_H4_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_GBP_JPY_H4()
    logging.info("Starting GBP/JPY H4 strategy monitoring...")
...
df_GBPJPY_H4["open"] = df_GBPJPY_H4["mid"].apply(lambda x: x["o"])
df_GBPJPY_H4["high"] = df_GBPJPY_H4["mid"].apply(lambda x: x["h"])
df_GBPJPY_H4["low"] = df_GBPJPY_H4["mid"].apply(lambda x: x["l"])
df_GBPJPY_H4["close"] = df_GBPJPY_H4["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_GBPJPY_H4 = df_GBPJPY_H4[["open", "high", "low", "close"]]
...
# MA optimized to GBP_JPY_H4
ma_pair = [(10, 70)]

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_GBPJPY_H4, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_GBP_JPY_H4()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_GBPJPY_H4)

# Create a new collection for signals if it doesn't exist
signals_collection = db["GBP_JPY_H4_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            ...
            "SMA_70": float(row["slow_ma"]),
            "SMA_10": float(row["fast_ma"]),
            "signal": int(row["signal"]),
