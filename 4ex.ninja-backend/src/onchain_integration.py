    ...
    async def get_token_balance(self, wallet_address: str) -> int:
        ...
        try:
            ...
            if not self.token_config.is_deployed:
                logger.info(f"Token not deployed, using simulation for {wallet_address}")
    ...
    def update_token_address(self, new_address: str) -> bool:
        ...
        try:
            ...
            if self.web3.is_connected():
                try:
                    contract = self.web3.eth.contract(address=new_address, abi=ERC20_ABI)
                    symbol = contract.functions.symbol().call()
                    decimals = contract.functions.decimals().call()
                    
                    logger.info(f"Contract verified - Symbol: {symbol}, Decimals: {decimals}")
