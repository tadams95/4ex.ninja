...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self.strict_transport_security and (self.is_production or request.url.scheme == "https"):
    ...
    def _add_https_security_headers(self, response: Response):
        ...
        response.headers["Expect-CT"] = (
            ...
            "report-uri=\"https://4ex.ninja/ct-report\""
        )
        ...
        if self.force_https:
            ...
            if not is_https:
                ...
                if not (
                    ...
                    request.url.path.startswith("/.well-known/acme-challenge/")
                ):
                    # Redirect to HTTPS
...
class SecurityMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    def _monitor_request_patterns(self, request: Request):
        ...
        if any(bot in suspicious_headers for bot in ["sqlmap", "nikto", "nmap", "dirb"]):
            logger.warning(
    ...
    def _monitor_response_patterns(self, request: Request, response: Response):
        ...
        if (
            request.url.path.startswith("/api/v1/auth/") and 
