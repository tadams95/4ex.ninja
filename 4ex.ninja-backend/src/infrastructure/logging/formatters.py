...
class PerformanceFormatter(BaseFormatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        timing_fields = ['duration', 'start_time', 'end_time', 'cpu_time', 'memory_usage']
        for field in timing_fields:
            if field in extra:
                performance_data[field] = extra[field]
        
        # Add request/response information if available
        request_fields = ['method', 'path', 'status_code', 'response_size', 'user_id', 'correlation_id']
...
class AuditFormatter(BaseFormatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Add action context
        action_fields = ['resource', 'resource_id', 'method', 'path', 'status', 'changes']
