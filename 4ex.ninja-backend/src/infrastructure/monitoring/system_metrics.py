...
class SystemMetricsMonitor:
    ...
    def get_metrics_summary(self, minutes: int = 60) -> Dict[str, Any]:
        ...
        return {
            ...
            "cpu": {
                "avg_percent": sum(m.cpu_percent for m in recent_metrics)
                / len(recent_metrics),
                "max_percent": max(m.cpu_percent for m in recent_metrics),
                "min_percent": min(m.cpu_percent for m in recent_metrics),
            },
            "memory": {
                "avg_percent": sum(m.memory_percent for m in recent_metrics)
                / len(recent_metrics),
            ...
            "processes": {
                ...
                "avg_count": sum(m.process_count for m in recent_metrics)
                / len(recent_metrics),
            },
        }
