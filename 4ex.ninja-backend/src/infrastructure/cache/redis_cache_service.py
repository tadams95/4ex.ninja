...
class RedisCacheService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logging.info(
                f"✅ Redis cache connected successfully to {self.host}:{self.port}"
            )
    ...
    def _get_cache_key(
        self, prefix: str, pair: str, timeframe: str, suffix: str = ""
    ) -> str:
    ...
    async def get_last_processed_time(
        ...
        try:
            cache_key = self._get_cache_key(
                self.KEY_PREFIX_LAST_PROCESSED, pair, timeframe
            )
            ...
            if timestamp_str:
                ...
                logging.debug(
                    f"📅 Cache hit: Last processed time for {pair}_{timeframe}: {last_time}"
                )
                ...
                logging.debug(
                    f"📅 Cache miss: No last processed time for {pair}_{timeframe}"
                )
    ...
    async def set_last_processed_time(
        self, pair: str, timeframe: str, timestamp: datetime
    ) -> bool:
        """
        ...
        try:
            ...
            await self.redis_client.setex(
                cache_key, self.CACHE_TTL_LAST_PROCESSED, str(timestamp_float)
            )

            logging.debug(
                f"📅 Cached last processed time for {pair}_{timeframe}: {timestamp}"
            )
    ...
    async def get_ma_state(
        self, pair: str, timeframe: str, ma_period: int
    ) -> Optional[Dict]:
        """
        ...
        try:
            cache_key = self._get_cache_key(
                self.KEY_PREFIX_MA_STATE, pair, timeframe, str(ma_period)
            )
            ...
            if cached_data:
                ...
                logging.debug(
                    f"📊 Cache hit: MA state for {pair}_{timeframe} MA{ma_period}"
                )
                ...
                logging.debug(
                    f"📊 Cache miss: No MA state for {pair}_{timeframe} MA{ma_period}"
                )
    ...
    async def set_ma_state(
        ...
        try:
            ...
            await self.redis_client.setex(
                cache_key, self.CACHE_TTL_MA_STATE, cached_data
            )
    ...
    async def update_ma_incremental(
        self, pair: str, timeframe: str, ma_period: int, new_price: float
    ) -> Optional[float]:
        """
        ...
        try:
            ...
            if not ma_state:
                logging.debug(
                    f"No cached MA state for incremental update: {pair}_{timeframe} MA{ma_period}"
                )
                return None
            ...
            logging.debug(
                f"📊 Incremental MA update: {pair}_{timeframe} MA{ma_period} = {new_ma:.5f}"
            )

            # Track incremental update
            safe_metrics_increment_counter(
                "ma_incremental_updates",
                1,
    ...
    async def clear_cache(
        self, pair: Optional[str] = None, timeframe: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            if pair and timeframe:
                ...
                if keys:
                    ...
                    logging.info(
                        f"🧹 Cleared cache for {pair}_{timeframe}: {len(keys)} keys"
                    )
                else:
                ...
                logging.info(
                    f"🧹 Cleared all signal processing cache: {total_cleared} keys"
                )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if not self.redis_client:
            health_status.update(
                {"status": "unavailable", "message": "Redis client not initialized"}
            )
            return health_status
