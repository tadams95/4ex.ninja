...
def validate_criteria():
    ...
    try:
        ...
        cors_response = requests.get(
            f"{BASE_URL}/", headers={"Origin": "http://localhost:3000"}
        )
        ...
        if "x-ratelimit-limit" in response.headers:
            print(
                f"   ✅ Rate Limiting: {response.headers['x-ratelimit-limit']} requests/window"
            )
            security_checks.append(True)
        else:
        ...
        if auth_response.status_code in [400, 401, 422, 429]:
            print(
                f"   ✅ Authentication: Properly secured (Status: {auth_response.status_code})"
            )
            security_checks.append(True)
        else:
            print(
                f"   ❌ Authentication: Unexpected response ({auth_response.status_code})"
            )
            security_checks.append(False)
