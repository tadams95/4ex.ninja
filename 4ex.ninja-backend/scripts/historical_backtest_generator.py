#!/usr/bin/env python3
"""
Historical Data Backtest Generator
Uses existing CSV historical data to run comprehensive backtests
...
class HistoricalDataBacktestGenerator:
    """Comprehensive backtest generator using existing historical CSV data."""
    ...
    def __init__(self):
        ...
        self.pairs = ["EUR_USD", "GBP_USD", "USD_JPY", "AUD_USD", "EUR_GBP", "GBP_JPY", "USD_CAD"]
        
        # Data directory
        self.data_dir = Path("backtest_results/historical_data")
        
        logger.info(f"🚀 Historical Data Backtest Generator initialized")
    ...
    def load_historical_data(self, pair: str) -> pd.DataFrame:
        ...
        csv_file = self.data_dir / f"{pair}_D_20230101_20241231.csv"
        
        if not csv_file.exists():
            logger.warning(f"⚠️ CSV file not found for {pair}: {csv_file}")
            return pd.DataFrame()
        ...
        try:
            ...
            if 'timestamp' in df.columns:
                ...
                # Use the first column as timestamp if no clear timestamp column
            ...
            for col in required_cols:
                if col not in df.columns:
                    logger.error(f"❌ Missing required column '{col}' in {pair} data")
                    return pd.DataFrame()
            ...
            # Convert to numeric and handle any string data
            ...
            logger.info(f"✅ Loaded {len(df)} records for {pair} from {csv_file.name}")
    ...
    def calculate_moving_averages(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Use boolean indexing properly - handle NaN values
        buy_condition = (df["prev_fast_above_slow"] == False) & (df["fast_above_slow"] == True)
        sell_condition = (df["prev_fast_above_slow"] == True) & (df["fast_above_slow"] == False)
    ...
    def _calculate_performance_metrics(
        ...
        # Calculate annual return properly
        years = len(df) / 252  # Assuming 252 trading days per year
        annual_return = (final_balance / initial_balance) ** (1 / years) - 1 if years > 0 else 0
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "volatility": round(returns.std() * np.sqrt(252), 4) if len(returns) > 0 else 0
            },
            "trade_statistics": {
            ...
            "equity_curve": [round(eq, 2) for eq in equity_curve[::max(1, len(equity_curve)//100)]]
        }
    ...
    def run_comprehensive_backtest(self):
        ...
        logger.info("🚀 Starting comprehensive historical data backtest...")
        
        results = {
            "backtest_metadata": {
                "generation_date": datetime.now(timezone.utc).isoformat(),
                "data_source": "Historical_CSV_Files",
                "strategy": self.strategy_name,
                "parameters": {
                    "fast_ma": self.fast_ma,
                    "slow_ma": self.slow_ma
                },
                "data_period": "2023-2024 (2 years)",
    ...
    def save_results(self, results: Dict[str, Any]):
        ...
        top_strategies = {
            ...
            "data_source": "Conservative Moderate Daily Strategy (MA 50/200) - Historical Data",
        ...
        for rank, result in enumerate(completed, 1):
            ...
            top_strategies["top_performing_strategies"].append({
                ...
                "category": "Conservative Moderate Daily",
                "description": f"MA crossover strategy ({self.fast_ma}/{self.slow_ma}) with historical validation"
            })
        
        # Save files
        files_to_save = [
            ("conservative_moderate_daily_backtest_results.json", results),
...
def main():
    ...
    generator = HistoricalDataBacktestGenerator()
