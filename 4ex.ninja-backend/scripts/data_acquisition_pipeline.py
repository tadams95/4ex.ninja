...
Target Pairs:
- Major Pairs (Priority 1): EUR/USD, GBP/USD, USD/JPY, USD/CHF, AUD/USD, USD/CAD
- Minor Pairs (Priority 2): EUR/GBP, EUR/JPY, GBP/JPY, AUD/JPY
...
class DataAcquisitionConfig:
    ...
    minor_pairs: Optional[List[str]] = None
...
class HistoricalDataAcquisition:
    ...
    async def _initialize_infrastructure(self):
        ...
        all_pairs = (self.config.major_pairs or []) + (self.config.minor_pairs or [])
    ...
    def _generate_download_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "data_completeness": (total_candles_downloaded / total_candles_expected)
            * 100,
    ...
    async def _validate_single_file(
        ...
        missing_pct = (missing_rows / (total_rows * len(df.columns))) * 100
    ...
    def _analyze_time_gaps(self, df: pd.DataFrame, timeframe: str) -> Dict[str, Any]:
        ...
        if timeframe == "4H":
            ...
            expected_interval = timedelta(weeks=1)
        else:
            expected_interval = timedelta(hours=1)

        # Find gaps
        time_diffs = df.index.to_series().diff()[1:]
        significant_gaps = time_diffs[time_diffs > expected_interval * 1.5]

        return {
            "gaps_found": len(significant_gaps),
            "max_gap_hours": (
                significant_gaps.max().total_seconds() / 3600
                if len(significant_gaps) > 0
                else 0
            ),
            "avg_gap_hours": (
                significant_gaps.mean().total_seconds() / 3600
                if len(significant_gaps) > 0
                else 0
            ),
        }
    ...
    def _validate_price_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_single_move_pct": price_changes.max() * 100,
            "valid_ohlc_pct": (
