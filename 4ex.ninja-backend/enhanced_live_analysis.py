                ...
                for candle in candles:
                    formatted_candles.append(
                        SwingCandleData(
                            ...
                            volume=(
                                int(candle.volume) if candle.volume is not None else 0
                            ),
                        )
                    )
...
async def run_comprehensive_regime_analysis():
    ...
    try:
        ...
        analysis_data = {"timestamp": datetime.now(), "timeframe": "4H"}
        ...
        trending_pairs = len(
            [
                p
                for p, data in market_result.get("individual_pairs", {}).items()
        ...
        print(
            f"   • Overall Market State: {'TRENDING' if market_result.get('is_trending', False) else 'RANGING'}"
        )
        print(
            f"   • Trending Pairs: {trending_pairs}/{total_pairs} ({trending_pct:.0f}%)"
        )
        print(
            f"   • Dominant Direction: {market_result.get('dominant_direction', 'MIXED').upper()}"
        )
        ...
        print(
            f"   • Volatility Percentile: {vol_result.get('volatility_percentile', 50):.0f}th percentile"
        )
        print(
            f"   • Normalized Level: {vol_result.get('normalized_volatility', 1):.2f}x normal"
        )
        print(
        ...
        print(
            f"   • Direction Split: {trend_dist.get('up_trends', 0)} up, {trend_dist.get('down_trends', 0)} down, {trend_dist.get('no_trend', 0)} ranging"
        )
        print()
        ...
        sentiment_text = (
            ...
            else "RISK-OFF" if sentiment_score < -0.2 else "NEUTRAL"
        )

        print(f"   • Risk Sentiment: {sentiment_text}")
        print(f"   • Sentiment Score: {sentiment_score:+.2f} (-1=risk-off, +1=risk-on)")
        print(
            f"   • Safe Haven Strength: {sentiment_result.get('safe_haven_strength', 0):+.2f}"
        )
        print(
            f"   • Risk Asset Performance: {sentiment_result.get('risk_asset_performance', 0):+.2f}"
        )
        print(f"   • USD Strength: {sentiment_result.get('usd_strength', 0):+.2f}")
        print(
            f"   • Correlation Breakdown: {'YES' if sentiment_result.get('correlation_breakdown') else 'NO'}"
        )
        print()
        ...
        print(
            f"   • Volatility Spike: {'DETECTED' if econ_result.get('volatility_spike_detected') else 'NONE'}"
        )
        print(
            f"   • Market Reaction: {econ_result.get('market_reaction_strength', 0):.2f}/1.0"
        )
        print(
            f"   • Synchronized Movement: {'YES' if econ_result.get('synchronized_movement') else 'NO'}"
        )
        print(
            f"   • Affected Currencies: {', '.join(affected) if affected else 'None'}"
        )
        print(
            f"   • Event Impact Score: {econ_result.get('event_impact_score', 0):.2f}"
        )
        print()
        ...
        regime_result = await detector.detect_current_regime(
            list(formatted_data.keys()), timeframe="4H"
        )
        ...
        sentiment_icons = {"risk_on": "💪🚀", "risk_off": "🛡️🔻", "neutral": "⚖️⏸️"}

        print(
            f"🎯 PRIMARY REGIME: {regime_icons.get(regime_result.regime.value, '📊')} {regime_result.regime.value.upper().replace('_', ' ')}"
        )
        print(
            f"😎 RISK SENTIMENT: {sentiment_icons.get(regime_result.sentiment.value, '⚖️')} {regime_result.sentiment.value.upper().replace('_', ' ')}"
        )
        ...
        for pair, candles in formatted_data.items():
            if len(candles) >= 2:
                ...
                market_state = (
                    "TRENDING" if pair_market.get("is_trending", False) else "RANGING"
                )
                ...
                print(
                    f"    Market: {market_state} {trend_dir} (strength: {trend_strength:.2f})"
                )
        ...
        print()
        print(
            f"🔄 Next regime evaluation: {regime_result.next_evaluation.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        print()
        print("✅ ENHANCED LIVE MARKET REGIME ANALYSIS COMPLETE!")
        print(
            "🎯 System successfully analyzed real-time market conditions with high precision!"
        )
