...
async def analyze_live_market_regimes():
    ...
    try:
        ...
        for pair in major_pairs:
            print(f"  📊 {pair}...", end="")
            try:
                candles = await data_infra.get_candles(pair, "4H", start_time, end_time)

                if candles and len(candles) > 20:  # Need sufficient data
                    market_data[pair] = candles
                    latest = candles[-1]
                    change = (
                        (float(latest.close) - float(candles[-2].close))
                        / float(candles[-2].close)
                    ) * 100
                    change_icon = "📈" if change > 0 else "📉"
                    print(
                        f" ✅ {len(candles)} candles | Latest: {latest.close} {change_icon} {change:+.2f}%"
                    )
                else:
                    print(
                        f" ⚠️ Insufficient data ({len(candles) if candles else 0} candles)"
                    )
        ...
        result = await detector.detect_current_regime(
            list(market_data.keys()), timeframe="4H"
        )
        ...
        sentiment_icons = {"risk_on": "💪", "risk_off": "🛡️", "neutral": "⚖️"}

        print(
            f"🎯 PRIMARY REGIME: {regime_icons.get(result.regime.value, '📊')} {result.regime.value.upper()}"
        )
        print(
            f"😎 RISK SENTIMENT: {sentiment_icons.get(result.sentiment.value, '⚖️')} {result.sentiment.value.upper()}"
        )
        ...
        components_data = {"timestamp": datetime.now(), "timeframe": "4H"}
        components_data.update(market_data)

        # Market Classification
        market_analysis = await detector.market_classifier.classify_market_condition(
            components_data
        )
        trending_pct = market_analysis.get("trending_percentage", 0) * 100
        print(f"📈 MARKET CONDITION:")
        print(
            f"   • Trending: {'YES' if market_analysis.get('is_trending') else 'NO'} ({trending_pct:.0f}% of pairs)"
        )
        print(
            f"   • Dominant Direction: {market_analysis.get('dominant_direction', 'N/A').upper()}"
        )
        print(
            f"   • Trend Strength: {market_analysis.get('trend_strength', 0):.2f}/1.0"
        )
        print()

        # Volatility Analysis
        vol_analysis = await detector.volatility_analyzer.analyze_volatility_regime(
            components_data
        )
        print(f"📊 VOLATILITY ANALYSIS:")
        print(f"   • Current Regime: {vol_analysis.get('regime', 'N/A').upper()}")
        print(
            f"   • Average Volatility: {vol_analysis.get('average_volatility', 0):.3f}"
        )
        print(
            f"   • Normalized Level: {vol_analysis.get('normalized_volatility', 1):.2f}x normal"
        )
        vol_dist = vol_analysis.get("regime_distribution", {})
        print(
            f"   • Distribution: {vol_dist.get('high', 0)} high, {vol_dist.get('normal', 0)} normal, {vol_dist.get('low', 0)} low"
        )
        print()

        # Trend Analysis
        trend_analysis = await detector.trend_analyzer.analyze_trend_strength(
            components_data
        )
        ...
        print(
            f"   • Pairs: {trend_dist.get('up_trends', 0)} up, {trend_dist.get('down_trends', 0)} down, {trend_dist.get('no_trend', 0)} ranging"
        )
        print()

        # Sentiment Analysis
        sentiment_analysis = await detector.sentiment_analyzer.analyze_risk_sentiment(
            components_data
        )
        print(f"😎 RISK SENTIMENT:")
        print(
            f"   • Sentiment Score: {sentiment_analysis.get('risk_sentiment_score', 0):+.2f} (-1=risk-off, +1=risk-on)"
        )
        print(
            f"   • Safe Haven Strength: {sentiment_analysis.get('safe_haven_strength', 0):+.2f}"
        )
        print(
            f"   • Risk Asset Performance: {sentiment_analysis.get('risk_asset_performance', 0):+.2f}"
        )
        print(
            f"   • Correlation Breakdown: {'YES' if sentiment_analysis.get('correlation_breakdown') else 'NO'}"
        )
        print()

        # Economic Events
        econ_analysis = await detector.economic_analyzer.analyze_event_impact(
            components_data
        )
        print(f"📰 ECONOMIC EVENT IMPACT:")
        print(f"   • High Impact Events: {econ_analysis.get('high_impact_events', 0)}")
        print(
            f"   • Volatility Spike Detected: {'YES' if econ_analysis.get('volatility_spike_detected') else 'NO'}"
        )
        print(
            f"   • Market Reaction Strength: {econ_analysis.get('market_reaction_strength', 0):.2f}/1.0"
        )
        affected = econ_analysis.get("affected_currencies", [])
        print(
            f"   • Affected Currencies: {', '.join(affected) if affected else 'None'}"
        )
        print()
        ...
        for pair, candles in market_data.items():
            if len(candles) >= 2:
                ...
                prev = candles[-2]
                change = (
                    (float(latest.close) - float(prev.close)) / float(prev.close)
                ) * 100
                change_icon = "📈" if change > 0 else "📉"

                # Get individual analysis if available
                individual_market = market_analysis.get("individual_pairs", {}).get(
                    pair, {}
                )
                individual_vol = vol_analysis.get("individual_pairs", {}).get(pair, {})
                individual_trend = trend_analysis.get("individual_pairs", {}).get(
                    pair, {}
                )

                trending = (
                    "📈 Trending"
                    if individual_market.get("is_trending")
                    else "📊 Ranging"
                )
                ...
                print(
                    f"  {pair}: {latest.close} {change_icon} {change:+.2f}% | {trending} {trend_dir} | Vol: {vol_regime}"
                )

        print()
        print(
            "🎯 Next regime evaluation scheduled for:",
            result.next_evaluation.strftime("%Y-%m-%d %H:%M:%S UTC"),
        )
        print()
