...
class MissingPairsFetcher:
    ...
    async def fetch_pair_data_chunked(
        self, pair: str, oanda_instrument: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            while (
                len(all_candles) < expected_periods and chunk_count < 100
            ):  # Safety limit
                chunk_count += 1
                ...
                if chunk_data:
                    ...
                    current_to_date = datetime.fromisoformat(
                        earliest_candle["timestamp"].replace("Z", "+00:00")
                    )
            ...
            logger.info(
                f"✅ {pair}: Fetched {len(filtered_candles)} H4 candles ({chunk_count} chunks)"
            )
    ...
    async def _fetch_chunk(
        self, instrument: str, to_time: datetime, count: int
    ) -> List[Dict]:
    ...
    async def save_pair_data(self, pair_data: Dict[str, Any]):
        ...
        try:
            if not pair_data.get("success", False):
                logger.error(
                    f"❌ Cannot save failed data for {pair_data.get('pair', 'unknown')}"
                )
                return
