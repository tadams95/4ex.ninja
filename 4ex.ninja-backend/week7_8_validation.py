...
class Week78Validator:
    ...
    def test_api_response_times(self) -> bool:
        ...
        for name, endpoint in endpoints.items():
            ...
            if response_times:
                ...
                print(
                    f"   {status} | Mean: {mean_time:.1f}ms | P95: {p95_time:.1f}ms | Range: {min_time:.1f}-{max_time:.1f}ms"
                )
                print(
                    f"   Success Rate: {((10-errors)/10)*100:.1f}% ({10-errors}/10 requests)"
                )
    ...
    def _test_security_headers(self) -> bool:
        ...
        try:
            ...
            optional_headers = ["Referrer-Policy", "Permissions-Policy"]
            ...
            if "Strict-Transport-Security" not in headers:
                ...
                print(
                    f"   ✅ Strict-Transport-Security: {headers['Strict-Transport-Security']}"
                )
    ...
    def _test_authentication(self) -> bool:
        ...
        try:
            ...
            if login_response.status_code in [400, 401, 422]:
                print(
                    f"   ✅ Authentication endpoint properly validates credentials (Status: {login_response.status_code})"
                )
                return True
            else:
                print(
                    f"   ❌ Unexpected auth response (Status: {login_response.status_code})"
                )
                return False
        ...
        try:
            ...
            for i in range(3):  # Just test a few requests
                ...
                if "x-ratelimit-limit" in response.headers:
                    print(
                        f"   ✅ Rate limit headers present: {response.headers.get('x-ratelimit-limit')} req/window"
                    )
                    return True
        ...
        try:
            ...
            for payload in malicious_payloads:
                ...
                if response.status_code in [
                    ...
                    422,
                    429,
                ]:  # 429 = rate limited (also protection)
                    print(
                        f"   ✅ Input validation working for payload: {payload['email'][:20]}..."
                    )
                else:
                    print(
                        f"   ❌ Input validation failed for: {payload['email'][:20]}..."
                    )
                    return False
    ...
    def _test_cors_configuration(self) -> bool:
        ...
        try:
            ...
            if (
                "access-control-allow-credentials" in cors_headers
                or "Access-Control-Allow-Credentials" in cors_headers
            ):
                credentials = cors_headers.get(
                    "access-control-allow-credentials"
                ) or cors_headers.get("Access-Control-Allow-Credentials")
            ...
            try:
                options_response = requests.options(
                    f"{BASE_URL}/api/v1/signals/", headers=headers
                )
                if options_response.status_code in [
                    200,
                    405,
                ]:  # 405 is OK if endpoint doesn't support OPTIONS
                    print(
                        f"   ✅ CORS preflight handling: {options_response.status_code}"
                    )
                    cors_checks.append(True)
                else:
                    print(
                        f"   ⚠️ CORS preflight unexpected: {options_response.status_code}"
                    )
    ...
    def generate_report(self) -> Dict:
        ...
        api_passed = all(
            result["passed"] for result in self.results["api_response_times"].values()
        )
        security_passed = (
            len(self.results["security_tests"]) > 0
        )  # If security tests ran

        # Calculate overall statistics
        if self.results["api_response_times"]:
            all_p95_times = [
                result["p95_ms"]
                for result in self.results["api_response_times"].values()
            ]
            overall_p95 = (
                statistics.quantiles(all_p95_times, n=20)[18]
                if len(all_p95_times) >= 20
                else max(all_p95_times)
            )
        else:
