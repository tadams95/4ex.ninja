...
class MovingAverageCrossStrategy_EUR_GBP_D:
    def __init__(
        self,
        slow_ma: int = 60,
        fast_ma: int = 10,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["EUR_GBP_D_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_EUR_GBP_D()
    logging.info("Starting EUR/GBP Daily strategy monitoring...")
...
df_EURGBP_D["open"] = df_EURGBP_D["mid"].apply(lambda x: x["o"])
df_EURGBP_D["high"] = df_EURGBP_D["mid"].apply(lambda x: x["h"])
df_EURGBP_D["low"] = df_EURGBP_D["mid"].apply(lambda x: x["l"])
df_EURGBP_D["close"] = df_EURGBP_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_EURGBP_D = df_EURGBP_D[["open", "high", "low", "close"]]
...
# MA optimized to EUR_GBP_D
ma_pair = [(60, 10)]

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_EURGBP_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_EUR_GBP_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_EURGBP_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["EUR_GBP_D_signals"]
