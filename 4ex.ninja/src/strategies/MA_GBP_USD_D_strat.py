...
collection = db["GBP_USD_D"]

collection_name = "GBP_USD_D"
...
class MovingAverageCrossStrategy_GBP_USD_D:
    def __init__(
        self,
        slow_ma: int = 80,  # Changed from 20
        fast_ma: int = 10,  # Changed from 10
        atr_period: int = 14,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["GBP_USD_D_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_GBP_USD_D()
    logging.info("Starting GBP/USD Daily strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_GBPUSD_D = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_GBPUSD_D.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_GBPUSD_D["open"] = df_GBPUSD_D["mid"].apply(lambda x: x["o"])
df_GBPUSD_D["high"] = df_GBPUSD_D["mid"].apply(lambda x: x["h"])
df_GBPUSD_D["low"] = df_GBPUSD_D["mid"].apply(lambda x: x["l"])
df_GBPUSD_D["close"] = df_GBPUSD_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_GBPUSD_D = df_GBPUSD_D[["open", "high", "low", "close"]]
...
# MA optimized to GBP_USD_D
ma_pair = [(10, 80)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_GBPUSD_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_GBP_USD_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_GBPUSD_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["GBP_USD_D_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            ...
            "SMA_80": float(row["slow_ma"]),  # Changed from SMA_60
            "SMA_10": float(row["fast_ma"]),  # Changed from SMA_40
            "signal": int(row["signal"]),
