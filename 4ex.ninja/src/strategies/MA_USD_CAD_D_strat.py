...
collection = db["USD_CAD_D"]

collection_name = "USD_CAD_D"
...
class MovingAverageCrossStrategy_USD_CAD_D:
    def __init__(
        self,
        slow_ma: int = 100,  # Changed from 40
        fast_ma: int = 40,   # Changed from 30
        atr_period: int = 14,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["USD_CAD_D_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_USD_CAD_D()
    logging.info("Starting USD/CAD Daily strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_USDCAD_D = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_USDCAD_D.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_USDCAD_D["open"] = df_USDCAD_D["mid"].apply(lambda x: x["o"])
df_USDCAD_D["high"] = df_USDCAD_D["mid"].apply(lambda x: x["h"])
df_USDCAD_D["low"] = df_USDCAD_D["mid"].apply(lambda x: x["l"])
df_USDCAD_D["close"] = df_USDCAD_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_USDCAD_D = df_USDCAD_D[["open", "high", "low", "close"]]
...
# MA optimized to USD_CAD_D
ma_pair = [(40, 100)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_USDCAD_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_USD_CAD_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_USDCAD_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["USD_CAD_D_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            ...
            "SMA_100": float(row["slow_ma"]),  # Changed from SMA_40
            "SMA_40": float(row["fast_ma"]),   # Changed from SMA_30
            "signal": int(row["signal"]),
