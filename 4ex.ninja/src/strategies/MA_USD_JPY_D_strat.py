...
collection = db["USD_JPY_D"]

collection_name = "USD_JPY_D"
...
class MovingAverageCrossStrategy_USD_JPY_D:
    def __init__(
        self,
        slow_ma: int = 20,  # Changed from 160
        fast_ma: int = 10,  # Changed from 20
        atr_period: int = 14,
    ...
    def generate_trade_dict(self, row: pd.Series) -> Optional[Dict]:
        ...
        try:
            ...
            sl_pips = abs(close_price - stop_loss) * Decimal(
                "100"
            )  # JPY pairs move in 0.01 increments
            tp_pips = abs(take_profit - close_price) * Decimal(
                "100"
            )  # JPY pairs move in 0.01 increments
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["USD_JPY_D_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_USD_JPY_D()
    logging.info("Starting USD/JPY Daily strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_USDJPY_D = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_USDJPY_D.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_USDJPY_D["open"] = df_USDJPY_D["mid"].apply(lambda x: x["o"])
df_USDJPY_D["high"] = df_USDJPY_D["mid"].apply(lambda x: x["h"])
df_USDJPY_D["low"] = df_USDJPY_D["mid"].apply(lambda x: x["l"])
df_USDJPY_D["close"] = df_USDJPY_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_USDJPY_D = df_USDJPY_D[["open", "high", "low", "close"]]
...
# MA optimized to USD_JPY_D
ma_pair = [(10, 20)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_USDJPY_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_USD_JPY_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_USDJPY_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["USD_JPY_D_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            ...
            "SMA_20": float(row["slow_ma"]),  # Changed from SMA_160
            "SMA_10": float(row["fast_ma"]),  # Changed from SMA_20
            "signal": int(row["signal"]),
