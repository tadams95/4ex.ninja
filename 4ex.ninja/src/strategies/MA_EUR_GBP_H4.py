...
collection = db["EUR_GBP_H4"]

collection_name = "EUR_GBP_H4"
...
class MovingAverageCrossStrategy_EUR_GBP_H4:
    def __init__(
        self,
        slow_ma: int = 90,  # Changed from 160
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["EUR_GBP_H4_signals"].find_one(sort=[("time", -1)])
                ...
                if new_prices:
                    ...
                    logging.info(
                        f"Processed H4 candles at {datetime.now(timezone.utc)}"
                    )

                # Sleep until next H4 candle
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_EUR_GBP_H4()
    logging.info("Starting EUR/GBP H4 strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_EURGBP_H4 = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_EURGBP_H4.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_EURGBP_H4["open"] = df_EURGBP_H4["mid"].apply(lambda x: x["o"])
df_EURGBP_H4["high"] = df_EURGBP_H4["mid"].apply(lambda x: x["h"])
df_EURGBP_H4["low"] = df_EURGBP_H4["mid"].apply(lambda x: x["l"])
df_EURGBP_H4["close"] = df_EURGBP_H4["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_EURGBP_H4 = df_EURGBP_H4[["open", "high", "low", "close"]]
...
# MA optimized to EUR_GBP_H4
ma_pair = [(90, 40)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_EURGBP_H4, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_EUR_GBP_H4()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_EURGBP_H4)

# Create a new collection for signals if it doesn't exist
signals_collection = db["EUR_GBP_H4_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            "time": index,
            "instrument": "EUR_GBP",  # Changed from AUD_USD
            "timeframe": "H4",
            "close": float(row["close"]),
            "SMA_90": float(row["slow_ma"]),  # Changed from 160
            "SMA_40": float(row["fast_ma"]),  # Changed from 50
            "signal": int(row["signal"]),
