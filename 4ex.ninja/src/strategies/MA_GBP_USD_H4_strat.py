...
collection = db["GBP_USD_H4"]

collection_name = "GBP_USD_H4"
...
class MovingAverageCrossStrategy_GBP_USD_H4:
    def __init__(
        self,
        slow_ma: int = 50,  # Changed from 60
        fast_ma: int = 40,  # Changed from 40
        atr_period: int = 14,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["GBP_USD_H4_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_GBP_USD_H4()
    logging.info("Starting GBP/USD H4 strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_GBPUSD_H4 = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_GBPUSD_H4.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_GBPUSD_H4["open"] = df_GBPUSD_H4["mid"].apply(lambda x: x["o"])
df_GBPUSD_H4["high"] = df_GBPUSD_H4["mid"].apply(lambda x: x["h"])
df_GBPUSD_H4["low"] = df_GBPUSD_H4["mid"].apply(lambda x: x["l"])
df_GBPUSD_H4["close"] = df_GBPUSD_H4["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_GBPUSD_H4 = df_GBPUSD_H4[["open", "high", "low", "close"]]
...
# MA optimized to GBP_USD_H4
ma_pair = [(40, 50)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_GBPUSD_H4, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_GBP_USD_H4()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_GBPUSD_H4)

# Create a new collection for signals if it doesn't exist
signals_collection = db["GBP_USD_H4_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            "time": index,
            "instrument": "GBP_USD",  # Changed from AUD_USD
            "timeframe": "H4",
            "close": float(row["close"]),
            "SMA_50": float(row["slow_ma"]),  # Changed from SMA_60
            "SMA_40": float(row["fast_ma"]),  # Changed from SMA_40
            "signal": int(row["signal"]),
