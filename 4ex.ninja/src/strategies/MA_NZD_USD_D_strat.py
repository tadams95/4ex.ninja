...
collection = db["NZD_USD_D"]

collection_name = "NZD_USD_D"
...
class MovingAverageCrossStrategy_NZD_USD_D:
    def __init__(
        self,
        slow_ma: int = 40,  # Changed from 110
        fast_ma: int = 30,  # Changed from 50
        atr_period: int = 14,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["NZD_USD_D_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_NZD_USD_D()
    logging.info("Starting NZD/USD Daily strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_NZDUSD_D = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_NZDUSD_D.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_NZDUSD_D["open"] = df_NZDUSD_D["mid"].apply(lambda x: x["o"])
df_NZDUSD_D["high"] = df_NZDUSD_D["mid"].apply(lambda x: x["h"])
df_NZDUSD_D["low"] = df_NZDUSD_D["mid"].apply(lambda x: x["l"])
df_NZDUSD_D["close"] = df_NZDUSD_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_NZDUSD_D = df_NZDUSD_D[["open", "high", "low", "close"]]
...
# MA optimized to NZD_USD_D
ma_pair = [(30, 40)]  # Changed MA periods

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_NZDUSD_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_NZD_USD_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_NZDUSD_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["NZD_USD_D_signals"]
...
for index, row in df_with_signals.iterrows():
    if row["signal"] != 0:  # Only save rows with actual signals
        signal_data = {
            ...
            "SMA_40": float(row["slow_ma"]),  # Changed from SMA_110
            "SMA_30": float(row["fast_ma"]),  # Changed from SMA_50
            "signal": int(row["signal"]),
