...
class MovingAverageCrossStrategy_AUD_USD_D:
    def __init__(
        self,
        slow_ma: int = 60,
        fast_ma: int = 40,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["AUD_USD_D_signals"].find_one(
                    sort=[("time", -1)]
                )
                ...
                if new_prices:
                    ...
                    logging.info(f"Processed daily candles at {datetime.now(timezone.utc)}")
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_AUD_USD_D()
...
df_AUDUSD_D["open"] = df_AUDUSD_D["mid"].apply(lambda x: x["o"])
df_AUDUSD_D["high"] = df_AUDUSD_D["mid"].apply(lambda x: x["h"])
df_AUDUSD_D["low"] = df_AUDUSD_D["mid"].apply(lambda x: x["l"])
df_AUDUSD_D["close"] = df_AUDUSD_D["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_AUDUSD_D = df_AUDUSD_D[["open", "high", "low", "close"]]
...
# MA optimized to AUD_USD_D
ma_pair = [(60, 40)]

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_AUDUSD_D, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_AUD_USD_D()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_AUDUSD_D)

# Create a new collection for signals if it doesn't exist
signals_collection = db["AUD_USD_D_signals"]
