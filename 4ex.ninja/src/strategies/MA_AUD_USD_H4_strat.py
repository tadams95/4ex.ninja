...
class MovingAverageCrossStrategy_AUD_USD_H4:
    def __init__(
        self,
        slow_ma: int = 160,
        fast_ma: int = 50,
    ...
    async def monitor_prices(self):
        ...
        while True:
            try:
                ...
                last_signal = db["AUD_USD_H4_signals"].find_one(sort=[("time", -1)])
...
if __name__ == "__main__":
    strategy = MovingAverageCrossStrategy_AUD_USD_H4()
    logging.info("Starting AUD/USD Daily strategy monitoring...")
    asyncio.run(strategy.monitor_prices())


# Fetch all documents and convert to DataFrame
df_AUDUSD_H4 = pd.DataFrame(list(collection.find()))

# Set the time column as the index
df_AUDUSD_H4.set_index("time", inplace=True)

# Extract OHLC values from the mid dictionary
df_AUDUSD_H4["open"] = df_AUDUSD_H4["mid"].apply(lambda x: x["o"])
df_AUDUSD_H4["high"] = df_AUDUSD_H4["mid"].apply(lambda x: x["h"])
df_AUDUSD_H4["low"] = df_AUDUSD_H4["mid"].apply(lambda x: x["l"])
df_AUDUSD_H4["close"] = df_AUDUSD_H4["mid"].apply(lambda x: x["c"])

# Drop unnecessary columns and reorder
df_AUDUSD_H4 = df_AUDUSD_H4[["open", "high", "low", "close"]]
...
# MA optimized to AUD_USD_H4
ma_pair = [(160, 50)]

# Apply to DataFrame
df_with_signals = identify_multiple_ma_crossovers(df_AUDUSD_H4, ma_pair)

# Initialize strategy
strategy = MovingAverageCrossStrategy_AUD_USD_H4()

# Calculate signals with SL/TP
df_with_signals = strategy.calculate_signals(df_AUDUSD_H4)

# Create a new collection for signals if it doesn't exist
signals_collection = db["AUD_USD_H4_signals"]
