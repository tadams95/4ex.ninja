...
function AccountPageContent() {
  ...
  useEffect(() => {
    ...
    if (user) {
      setName(user.name || '');
      setEmail(user.email || '');
      // Note: Subscription status now handled by wallet token balance
      // No need to fetch from traditional subscription API
    }
  }, [loading, isAuthenticated, router, user]);

  // Token access is now handled by wallet connection (no subscription fetching needed)
  const fetchSubscriptionStatus = async () => {
    setSubscriptionLoading(true);
    try {
      // Wallet-based access - no API call needed
      // Token balance and tier determined by wallet connection
      setIsSubscribed(false); // Default to false until token system is active
      setSubscriptionEnds(null);
    } catch (error) {
      console.error('Error checking access status:', error);
      setMessage({
        type: 'error',
        text: 'Failed to load access details. Please refresh the page.',
      });
    } finally {
