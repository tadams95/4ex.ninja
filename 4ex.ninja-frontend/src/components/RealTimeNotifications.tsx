...
export default function RealTimeNotifications({
  ...
  const { notifications, unreadCount, markAsRead, clearNotifications, connectionState } =
    useNotificationConnection();
          ...
          {filteredNotifications.slice(0, 5).map((notif, index) => (
            <div key={`${notif.data?.id}-${index}`} className="bg-gray-800 rounded p-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="text-gray-400 text-xs">{notif.data?.confidence}%</span>
              </div>
            </div>
          ))}
    ...
    <div className="bg-gray-900 rounded-lg p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button onClick={clearNotifications} className="text-red-400 hover:text-red-300 text-sm">
            Clear All
          </button>
        </div>
      </div>
          ...
          {['ALL', 'BUY', 'SELL', 'ALERT'].map(type => (
            <button
              ...
              className={`px-3 py-1 rounded text-sm font-medium transition-colors ${
                filter === type ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-white'
              }`}
            >
              {type}
            </button>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {notif.data?.message && (
                    <p className="text-gray-300 mt-2 text-sm">{notif.data.message}</p>
                  )}
                </div>
